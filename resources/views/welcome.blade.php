...
<head>
    ...
    <style>
        ...
        #printerParts {width:100%;border-collapse: collapse;min-width:660px;}
        #printerParts td:nth-child(1), #printerParts th:nth-child(1) {padding-left:8px;max-width: 600px;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;}
        #printerParts td:nth-child(4), #printerParts td:nth-child(5),
        #printerParts th:nth-child(4), #printerParts th:nth-child(5) {text-align: right; padding-right: 8px;}
        #printerParts thead {background-color: #6c7ae0; color: white;height: 34px;}
        #printerParts tbody tr td {border: solid 1px #d8d8d8;border-left: 0;border-right: 0;}
        #printerParts tbody tr:nth-child(2n) {background-color: whitesmoke;}
        #detailsModal .modal-dialog {max-width: calc(100% - 22px);min-width:690px;}
                ...
                @foreach($printers as $printer)
                    <div class="p-2">
                        <div class="card shadow sm:rounded-lg">
                            <div class="card-header">{{ $printer->name }}</div>
                            <div class="card-body">
                                <button type="button" class="btn btn-dark btn-sm" data-name="{{ $printer->name }}">Show details</button>
                            </div>
                        </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <table id="printerParts">
                    <thead>
                        <tr><th>Name</th><th>Type</th><th>Color</th><th>Yield</th><th>Price</th></tr>
                    </thead>
...
<script>
    ...
    (function() {
        ...
        const submitListeners = {
            "add-printer-form": function(form) {
                api.post('/printers', {term:form[1].value})
                    .then((reply) => {
                        console.log({reply});
                    })
                    .catch((error) => {
        ...
        window.addEventListener('load', function() {
            $('button.btn.btn-dark.btn-sm').click(function(e) {
                const term = e.target.dataset.name;
                api.post('/printers', {term:term})
                    .then((reply) => {
                        let html = '';
            ...
            const validation = Array.prototype.filter.call(forms, function(form) {
                form.addEventListener('submit', function(event) {
                    if (form.checkValidity() === false) {
                        ...
                        if(submitListeners[form.id]) {
                            event.preventDefault();
                            submitListeners[form.id](form);
                        }
                    }
